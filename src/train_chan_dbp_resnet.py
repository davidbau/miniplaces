...
class ChanDoubleBackpropLoss(object):
    ...
    def __call__(self, inp, out_with_extra, target):
        ...
        grad_inp = 0
        max_features = [f.view(f.shape[0], f.shape[1], -1).max(2)[0].sum()
                for f in features]
...
def main():
    ...
    experiment_dir = 'experiment/chan_3_resnet_qcrop'
    # Here's our data
    ...
    max_iter = 50000
    criterion = ChanDoubleBackpropLoss(1e3)
